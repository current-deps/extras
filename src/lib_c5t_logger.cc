    ...
    inner_logger_.MutableUse([&](InnerLoggerImpl& inner) {
      ...
      if (create_or_recreate) {
        ...
        if (log_starting_new && Exists(log_name_)) {
          ...
          auto const to_ignore_since_explicit_static_cast_void_does_not_work = ::symlink(dst.c_str(), src.c_str());
          static_cast<void>(to_ignore_since_explicit_static_cast_void_does_not_work);
        }
      }
      if (log_starting_new) {
